...
public:
    ...
    VMNodeState(const VMNodeState& state) : VMObject(QtNodes::NodeDataType{ "0","Bool" }, VMDATATYPE::VM_BOOL) {
        ...
        this->orderId = state.orderId; // 复制新变量
    }
    ...
    VMNodeState& operator = (const VMNodeState& state) {
        if (this != &state) {
            this->nodeState = state.nodeState;
            this->orderId = state.orderId; // 赋值新变量
        }
        return *this;
    }
...
public:
    ...
    VMRectangle(const VMRectangle& rectangle) : VMObject(QtNodes::NodeDataType{ "2","Rectangle" }, VMDATATYPE::VM_RECTANGLE) {
        ...
        this->orderId = rectangle.orderId; 
    }

    VMRectangle(const int& w, const int& h, const VMPoint2d& p, const float& angl, int order = 0)
        : VMObject(QtNodes::NodeDataType{ "2","Rectangle" }, VMDATATYPE::VM_RECTANGLE),
        width(w), height(h), centerPoint(p), angle(angl), orderId(order) {
        ;
    }
    ...
    VMRectangle& operator = (const VMRectangle& rectangle) {
        if (this != &rectangle) {
            ...
            this->centerPoint = rectangle.centerPoint;
            this->angle = rectangle.angle;
            this->orderId = rectangle.orderId; // 赋值 orderId
        }
        return *this;
    }
...
using PtrVMPolygon = std::shared_ptr<VMPolygon>;
...
template <typename T1, typename T2, typename = std::enable_if_t<std::is_same<T1, int>::value>>

class VMUnorderedMap :public VMObject
{
public:
    using Ptr = std::shared_ptr<VMUnorderedMap<T1, T2>>;
    VMUnorderedMap() :VMObject(QtNodes::NodeDataType{ "101","UnorderedMap" }, VMDATATYPE::VM_UNORDEREDMAP) { ; }
    VMUnorderedMap(const VMUnorderedMap<T1, T2>& map) :VMObject(QtNodes::NodeDataType{ "101","UnorderedMap" }, VMDATATYPE::VM_UNORDEREDMAP) { this->vmMap = map.vmMap; }
    VMUnorderedMap(const std::unordered_map<T1, T2>& map) :VMObject(QtNodes::NodeDataType{ "101","UnorderedMap" }, VMDATATYPE::VM_UNORDEREDMAP) { this->vmMap = map; }
