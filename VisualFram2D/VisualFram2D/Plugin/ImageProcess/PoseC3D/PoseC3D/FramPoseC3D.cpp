...
FramPoseC3D::FramPoseC3D(QWidget* parent)
...
FramPoseC3D::~FramPoseC3D()
{}

void FramPoseC3D::Initial()
{
    InitialNodeData();
    InitialImageShow();
    InitialPushButton();
    ...
    ptrPoseC3DAlgorithm = std::make_shared<PoseC3DAlgorithm>();
}

void FramPoseC3D::InitialNodeData()
{
    ...
    ptrInVMUnorderedMap = std::make_shared<VMUnorderedMap<int, PtrVMPolygon>>();
    ptrInNodeVMUnorderedMap = std::make_shared<BaseNodeData>();
    //ptrInNodeVMMat->SetCGObjectNodeFlow(ptrInVMMat);
    ptrInNodeVMUnorderedMap->AddScopeNodeDataType(QtNodes::NodeDataType{ "101","UnorderedMap" });

    FlowNodeAndName in_node_and_name;
    in_node_and_name.nodeName = "预测结果";
    in_node_and_name.ptrNodedata = ptrInNodeVMUnorderedMap;
    mapCGObjectFlowInNodeAndName.insert(std::make_pair(0, in_node_and_name));
}

void FramPoseC3D::InitialPushButton()
{
    connect(ui.pushButtonExecute, &QPushButton::clicked, this, [this]() {
        isSelfRunring = true;
        RunningTool();
        });  //按钮按下后信号发出执行操作
    connect(ui.pushButtonOpenModel, &QPushButton::clicked, this, &FramPoseC3D::PushButtonOpenModelClicked);
}

void FramPoseC3D::InitialImageShow()
{
    this->setWindowTitle("行为识别模型推理");
    this->setWindowIcon(QIcon(":/FramPoseC3D/Image/iconRecognation.png"));
    // 去掉问号，只保留关闭
    ...
    connect(this, &FramPoseC3D::SendNodeRuningStateSyncSig, this, &FramPoseC3D::AcceptSelfRunringStateSlot);
    connect(ui.spinBoxClassNum, QOverload<int>::of(&QSpinBox::valueChanged), this, &FramPoseC3D::SpinBoxValueChange);
}

void FramPoseC3D::InitilaToolParamUI()
{
    //初始化表格
...
void FramPoseC3D::InitialExserializeUi()
{
    connect(this, &FramPoseC3D::SendExserializeFinishSig, this, [this]() {
    ...
    connect(this, &FramPoseC3D::SendExserializeFinishSig, this, [this]() {
        ...
        for (int i = 0; i < classNum; ++i) { 
            qTableViewModel->setItem(i, 1, new QStandardItem(modelClassName[i])); 
        }
        });
    //初始化预测模型
    connect(this, &FramPoseC3D::SendExserializeFinishSig, this, [this]() {
        ...
        if (!trtFile.empty()) { 
            detector.InitializePoseC3D(trtFile); }
        });
}

int FramPoseC3D::RunningTool()
{
    ...
    ptrInVMUnorderedMap = ptrInNodeVMUnorderedMap->GetCGObjectNodeFlow<VMUnorderedMap<int, PtrVMPolygon>>();
    ptrOutVMUnorderedMap->state = false;

    if (ptrInVMUnorderedMap == nullptr || ptrInVMUnorderedMap->vmMap.size() == 0 || ptrInVMUnorderedMap->state == false) {
    ...
    //获取行为识别模型预测结果
    //int returnState = ptrPoseC3DAlgorithm->RunPoseC3D(detector, functionParam);
    //if (returnState != 0) {
    //    qDebug() << "Model load failed.";
    //}
    //else {
    //    //yoloV11ResultVMUnorderedMap = ptrPoseC3DAlgorithm->GetInferResult();
    //}
...
void FramPoseC3D::AcceptNodeBindingSucceedSlot(const QtNodes::PortIndex port_index)
{
...
void FramPoseC3D::PushButtonOpenModelClicked()
{
...
void FramPoseC3D::SpinBoxValueChange(int value)
{
...
void FramPoseC3D::AcceptSelfRunringStateSlot(const float& time, const RUNNINGSTATE& run_state)
{
    //获取模型输出图像
    //displayMat = ptrPoseC3DAlgorithm->GetInferImage();
    QString qstr_des = "状态: ";
    switch (run_state) {
    case RUNNING_SUCESS: {
    ...
    //更新表格数据
    /*if (yoloV11ResultVMUnorderedMap->vmMap.size() > 0) {
        for (int i = 0; i < ui.spinBoxClassNum->value(); ++i) {
            qTableViewModel->setItem(i, 2, new QStandardItem(QString::number(yoloV11ResultVMUnorderedMap->vmMap[i].state)));
        }
    }*/
    classNum = ui.spinBoxClassNum->value();
    modelClassName.clear();
...
int FramPoseC3D::OnceExecute()
{
    isSelfRunring = false;
    std::thread thrad = std::thread(&FramPoseC3D::RunningTool, this);
    thrad.detach();
    return 0;
}

void FramPoseC3D::SetNodeId(const int node_id)
{
    this->nodeId = node_id;
...
QJsonObject FramPoseC3D::SerializeFunciton()
{
...
void FramPoseC3D::ExserializeFunciton(const QJsonObject& json)
{
