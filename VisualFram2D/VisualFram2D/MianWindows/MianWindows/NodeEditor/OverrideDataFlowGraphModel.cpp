...
namespace QtNodes {
    ...
    DataFlowGraphModelOverride::DataFlowGraphModelOverride(std::shared_ptr<NodeDelegateModelRegistry> registry)
        : _registry(std::move(registry))
        , _nextNodeId{ 0 }
    {
        FrmLog* ptr_framlog = FrmLog::FrmLogInstance();
        QObject::connect(this, &DataFlowGraphModelOverride::SendLogInfoSynSig, ptr_framlog, &FrmLog::LogMassageShowSlot);
    }

    std::unordered_set<NodeId> DataFlowGraphModelOverride::allNodeIds() const
    {
    ...
    std::unordered_set<ConnectionId> DataFlowGraphModelOverride::allConnectionIds(NodeId const nodeId) const
    {
    ...
    std::unordered_set<ConnectionId> DataFlowGraphModelOverride::connections(NodeId nodeId,
        PortType portType,
    ...
    bool DataFlowGraphModelOverride::connectionExists(ConnectionId const connectionId) const
    {
        return (_connectivity.find(connectionId) != _connectivity.end());
    }

    NodeId DataFlowGraphModelOverride::addNode(std::unique_ptr<NodeDelegateModel>& model)
    {
        if (model) {
            ...
            connect(model.get(),
                ...
                this,
                &DataFlowGraphModelOverride::portsDeleted);
            ...
            connect(model.get(),
                ...
                this,
                &DataFlowGraphModelOverride::portsInserted);

            BaseNodeModel* ptr = dynamic_cast<BaseNodeModel*>(model.get());
            ptr->SetNodeId(newId);
    ...
    NodeId DataFlowGraphModelOverride::addNode(QString const nodeType)
    {
        std::unique_ptr<NodeDelegateModel> model = _registry->create(nodeType);
        if (model == nullptr) { 
            emit SendLogInfoSynSig(nodeType + "数据节点不存在! ", LOGTYPE::ERRORS);
            return InvalidNodeId;
        }
        ...
        if (model) {
            ...
            BaseNodeModel *ptr = dynamic_cast<BaseNodeModel*>(model.get());
            auto  ptr_interface = ptr->GetToolInterface();
            ptr_interface->SetToolNodeDataInteract(toolNodeDataInteract);
            ptr_interface->SetToolNodeMapData(ptrMapNodesOutData);

            ptr->SetNodeId(newId);
            //ptr_interface->TransferExDeviceData(StaticGlobalVariable::varPtrMapQueuePtrExDeviceData);
            //ptr_interface->SetNodeLogData(StaticGlobalVariable::varPtrQueueNodeLogData);
    ...
    bool DataFlowGraphModelOverride::connectionPossible(ConnectionId const connectionId) const
    {
    ...
    void DataFlowGraphModelOverride::addConnection(ConnectionId const connectionId)
    {
    ...
    void DataFlowGraphModelOverride::sendConnectionCreation(ConnectionId const connectionId)
    {
    ...
    void DataFlowGraphModelOverride::sendConnectionDeletion(ConnectionId const connectionId)
    {
    ...
    bool DataFlowGraphModelOverride::nodeExists(NodeId const nodeId) const
    {
        return (_models.find(nodeId) != _models.end());
    }

    QVariant DataFlowGraphModelOverride::nodeData(NodeId nodeId, NodeRole role) const
    {
        QVariant result;
    ...
    NodeFlags DataFlowGraphModelOverride::nodeFlags(NodeId nodeId) const
    {
        auto it = _models.find(nodeId);
    ...
    bool DataFlowGraphModelOverride::setNodeData(NodeId nodeId, NodeRole role, QVariant value)
    {
        Q_UNUSED(nodeId);
        Q_UNUSED(role);
        Q_UNUSED(value);
    ...
    QVariant DataFlowGraphModelOverride::portData(NodeId nodeId,
        PortType portType,
        PortIndex portIndex,
        PortRole role) const
    {
    ...
    bool DataFlowGraphModelOverride::setPortData(
    ...
    bool DataFlowGraphModelOverride::deleteConnection(ConnectionId const connectionId)
    {
    ...
    bool DataFlowGraphModelOverride::deleteNode(NodeId const nodeId)
    {
    ...
    QJsonObject DataFlowGraphModelOverride::saveNode(NodeId const nodeId) const
    {
        QJsonObject nodeJson;
    ...
    QJsonObject DataFlowGraphModelOverride::save() const
    {
    ...
    void DataFlowGraphModelOverride::loadNode(QJsonObject const& nodeJson)
    {
        ...
        if (model == nullptr) {
            emit SendLogInfoSynSig(delegateModelName + ": 数据节点序列化失败，数据节点未注册! ", LOGTYPE::ERRORS);
            return ;
        }
        ...
        if (model) {
            ...
            /*
            添加信槽函数部分，原始反序列化，没有进行删除和节点功能的绑定
            */
            connect(model.get(),
                &NodeDelegateModel::portsAboutToBeDeleted,
                this,
    ...
    void DataFlowGraphModelOverride::load(QJsonObject const& jsonDocument)
    {
    ...
    void DataFlowGraphModelOverride::onOutPortDataUpdated(NodeId const nodeId, PortIndex const portIndex)
    {
    ...
    void DataFlowGraphModelOverride::propagateEmptyDataTo(NodeId const nodeId, PortIndex const portIndex)
    {
        QVariant emptyData{};
