...
class FramImageDuration : public ToolInterface, private PublicFunction
{
    Q_OBJECT

public:
    FramImageDuration(QWidget* parent = nullptr);
    ~FramImageDuration();
...
private:
    ...
    //输入:2 
    PtrVMUnorderedMap<int, PtrVMObject>ptrInVec = {nullptr};
    ...
    /// <summary>
    /// 参数方法
    VideoInfo MethodParam;
    FunctionParam functionParam; //算法结构参数
    UiParam uiParam; //界面参数
    bool isSelfRunring{ true };
    ImageDurationAlgorithm::Ptr ptrImageDurationAlgorithm;

private:
    int clearCount = 1;

private:
    Ui::FramImageDurationClass ui;
};
